...
def cards():
    ...
    dbs = [
        f for f in os.listdir(
            pathDB,
        ) if os.path.isfile(os.path.join(pathDB, f))
    ]
    dbs = list(filter(lambda k: k.endswith('.db'), dbs))
...
def filter_cards(filter_name):
    ...
    query = filters.get(filter_name, 'where 1 = 1')
    if (query is None):
        query = f'where type = {filter_name}'
...
def add_card():
    ...
    print(request.form)
    db.execute(
        'INSERT INTO cards (type, front, back) VALUES (?, ?, ?)',
        [
            request.form['type'],
            request.form['front'],
            request.form['back'],
...
def memorize(card_type, card_id=None):
    # TODO: website crashes when going here without selection db first
    #       redirect redirects incorrectly
    # if nameDB == 'None':
    ...
    if card_id:
        ...
        card, is_empty_set = get_card(card_type)
    if not card:
        if not is_empty_set:
    ...
    return render_template(
        ...
        card=card,
        card_type=card_type,
        short_answer=short_answer, tags=tags, current_db=nameDB,
    )
...
def edit_tag(tag_id):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    tag = getTag(tag_id)
    return render_template('editTag.html', tag=tag, current_db=nameDB)
...
def create_db():
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    return render_template('createDb.html', current_db=nameDB)
